...
const owner = JSON.parse(fs.readFileSync('./dataRaffX/owner.json').toString())
...
auth: state,
browser: [`Jadibot Md By (Teams Bot RaffX)`, "Chrome", "1.0.0"],
...
require('./raffxdata')(kayla, m, chatUpdate, store)
} catch (err) {
console.log(err)}
})
...
sendMessage(`6281328139682@s.whatsapp.net`,{text: txt, mentions : [user]})
}
if (connection === 'close') {
